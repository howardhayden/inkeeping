...
6. Import, review, apply, explicit save, local-checkpoint acceptance, diagnostic receipt/witness-request export, signed witness/policy selection, separate policy-pin entry, backup download, report download, and recovery review work with synthetic records in a disposable browser profile.
7. Attempt missing/substituted policy pins, attacker-supplied policy, deleted/truncated/duplicated/forked/origin-spliced witnesses, and unknown/revoked/invalid signatures; every ordinary action must block distinctly. Confirm an unsigned local receipt never releases output.
8. In two same-origin tabs, delay or suppress notification and change the named generation before each ordinary whole-catalog/per-record catalog/archive/service/operational/Public Notice action, during a maximum representative build, and while the second state read is pending. The action must reject token/recovery/state/audit/revision/anchor/external-continuity/evidence/artifact disagreement and issue no browser activation request.
9. From a clean exact signed generation, confirm content is rendered into an immutable file from the reopened saved snapshot, survives both artifact digests, and invokes synchronous activation only while the final readonly manifest/generation/anchor fence matches. Confirm a writer begun afterward is queued until the activation request returns. Confirm the Technical Report remains a clearly labeled diagnostic when the open session is stale, unsaved, or unattached.
10. Confirm each supported browser actually opens or saves the requested file after the activation request; record popup or automatic-download blocking, retry behavior, filename, and file digest. An anchor click alone is not evidence of a durable file.
11. No test workspace is treated as migratable production data.

The final readonly transaction prevents a later IndexedDB writer from committing between final saved-state verification and the synchronous browser activation request. It does not prove that the browser or operating system saved or opened the file, prevent a write after the request returns, provide continuing freshness, govern an already downloaded file, or authorize publication.
