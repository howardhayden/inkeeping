...
A shared UI gate blocks ordinary whole-catalog, per-record catalog, archive, service, operational, and public outputs while full snapshot validation is incomplete or failed; the session is unnamed, dirty, drafted, busy, known stale, or quarantined; sample content or blocking findings remain; active operator-admitted-unverified evidence or unattributed catalog/archive/service content exists; or the exact named generation lacks a signed witness terminal under the exact current policy digest obtained through the separate governed channel. Unsigned receipts remain diagnostic. Whole-catalog output additionally requires at least one record, matching the documented import contract, so a blank workspace cannot emit a successful-looking self-incompatible package.

The render-time gate is not treated as continuing freshness proof. Each ordinary action reopens and validates the named saved generation, rejects fallback recovery, compares identity/token/state/audit/revision/anchor/external-continuity/evidence with the current session, and constructs an immutable `File` from that saved snapshot rather than the React closure. It re-digests the artifact before and after the second state read. The storage layer accepts only that exact file and its disposition. The final exact manifest/generation/anchor comparison and synchronous Blob/anchor activation request share one readonly IndexedDB transaction. A delayed notification or earlier commit therefore cannot authorize a stale artifact, and a later writer waits until the request returns. The Technical Report remains a diagnostic rendering; current-session conflict-recovery backup remains a separate explicit path.

This is a bounded database-to-browser-request control, not an atomic publication transaction. Another browser context can commit after the request returns, and the browser or operating system may block or fail the new-tab/download action. The control establishes correspondence to one exact saved generation through synchronous activation request, not that the file was persisted or remains latest when later opened, transferred, approved, or published. Browser task testing, actual file confirmation, destination review, and institutional approval remain required.
...
The Public Notice is constructed from an allowlisted projection of nonsynthetic open-incident service categories from the exact reopened saved snapshot. It does not receive workspace name, raw incident evidence/notes, catalog/archive/service records, configuration, hashes, or staff-role values. Any synthetic record or incident blocks generation regardless of state; unsupported closure, invalid/unchecked state, blocking findings, stale/dirty/drafted/busy/unnamed/recovery/quarantine state, active admitted/unattributed content, or anything short of exact signed external `trusted-match` under the separately supplied current policy digest also blocks the UI action. The immutable notice bytes and final saved state are checked again before synchronous activation. This minimizes technical exposure but does not prove browser/OS completion, authorize publication, or prove later currency. Each notice requires communications, privacy, accessibility, and service-owner review.
