...
| Browser-local persistence | [`app/lab-storage.ts`](../../app/lab-storage.ts), [ADR 0003](../decisions/0003-manifest-bound-two-generation-storage.md), storage tests | Explain atomic manifest/generation transactions, optimistic tokens, digest binding, bounded inspection, and non-destructive reconstruction | IndexedDB is plaintext, origin-scoped, evictable, and not institutional backup/preservation |
| Data integrity design | Revision/audit code in [`app/lab-core.ts`](../../app/lab-core.ts), [`app/workspace-backups.ts`](../../app/workspace-backups.ts), [Threat model](../THREAT_MODEL.md) | Distinguish revision, state, event, payload, and file digests and verify failure behavior | Hashes do not authenticate operators, anchor time, prevent a writer's recomputation, or prove custody/nonrepudiation |
| Defensive import engineering | [`app/xml-safety.ts`](../../app/xml-safety.ts), import paths in [`app/lab-core.ts`](../../app/lab-core.ts), [ADR 0004](../decisions/0004-fail-closed-import-quarantine.md) | Demonstrate fail-closed XML/JSON/RIS/MARC/BibTeX/tabular parsing, exact bounds, and no silent reduction | Structural acceptance is not semantic accuracy, source authority, or safety of a manually opened URL |
...
| MARC | MARCXML and mnemonic code/tests | Leader/tag/indicator/subfield grammar, source evidence, conservative type/name mapping, escaping and malformed-line refusal |
| Dublin Core / MODS | Parsers/serializers and exact-structure tests | Namespace/profile paths, date/right/license/descriptive mappings, unsupported extension handling |
| JSON ecosystems | Native packet, CSL-JSON, Schema.org JSON-LD, backup/schema/service JSON | Exact envelope/versioning, DTO reconstruction, prototype-key refusal, bounded structures, literal date/name tradeoffs |
| Tagged/bibliographic text | RIS and bounded BibTeX parser/serializer tests | Record/line termination, nested braces, safe grammar, macro/concatenation refusal, injection/escaping |
...
| Archival standards | EAD variants, DCTAP, AtoM, ArchivesSpace cues, RiC mapping fields | Version/profile differences, description hierarchy, data-type/cardinality documentation, vendor boundary |
...
| Recovery care | Manifest-bound prior, quarantine, new-UUID reconstruction, source preservation | Recover without concealing corruption or destroying evidence; separate recovery from authorized disposal |
