...
| Browser-local persistence | [`app/lab-storage.ts`](../../app/lab-storage.ts), [`app/continuity-anchor.ts`](../../app/continuity-anchor.ts), [`app/external-continuity.ts`](../../app/external-continuity.ts), [ADR 0003](../decisions/0003-manifest-bound-two-generation-storage.md), [ADR 0007](../decisions/0007-signed-continuity-and-activation-fences.md), storage tests | Explain atomic save transactions, optimistic tokens, local diagnostic anchors/receipts, signed witness/pinned-policy verification, final readonly activation fence, bounded inspection, and non-destructive reconstruction | Browser-local stores are plaintext, origin-scoped, evictable, and coherently rewritable; ordinary output additionally needs an externally signed terminal and separately obtained current policy digest, whose authority/custody the app cannot establish |
| Data integrity design | Revision/audit code in [`app/lab-core.ts`](../../app/lab-core.ts), [`app/continuity-anchor.ts`](../../app/continuity-anchor.ts), [`app/external-continuity.ts`](../../app/external-continuity.ts), [`app/workspace-backups.ts`](../../app/workspace-backups.ts), [Threat model](../THREAT_MODEL.md) | Distinguish revision, state, event, payload, file, local checkpoint, signed witness, policy, warning-manifest, and recovery-review digests; verify exact successor/topology/mismatch behavior | Internal hashes and unsigned receipts do not authenticate; signed correspondence under a supplied policy pin still does not establish source truth, completeness, trusted time, signer authority, custody, or nonrepudiation |
