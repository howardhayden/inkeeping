...
9. Exercise invalid/unchecked, finding, unnamed, dirty, recovery, stale, unanchored/local-only/unsigned/missing-policy-pin/invalid-signature/truncated/forked/continuity-mismatch, and admitted/unattributed-evidence states; verify every ordinary catalog, archive, service, operational, and public path fails closed. Verify zero-record catalog output also rejects.
10. Suppress/delay cross-tab notification; change the named generation immediately before click, during a deliberately large artifact build, and while the second read is pending; mutate the constructed artifact; and attempt concurrent lease reuse. Verify earlier changes and artifact mutation block, the lease is single-use, a later IndexedDB writer waits until the synchronous browser activation request returns, and no activation request occurs on failure. Separately observe whether the browser/OS actually opens or saves the file.
...
Acceptance: state changes are revision/audit bound; closure/target are explicit; drafts are safe; Technical Report is a bounded diagnostic rendering; ordinary artifacts match the exact signed saved checkpoint through the synchronous activation request and fail on detected races; Public Notice contains only the fixed projection; browser/OS completion, later currency, and human publication approval remain explicit.

### H. Workspace, backup, recovery, and deletion

1. Create a named workspace; save; confirm it is unanchored and ordinary output blocks. Explicitly accept a baseline with the required continuity-not-authenticity acknowledgment; download its diagnostic receipt and unsigned witness request; modify/save; confirm the anchor advances atomically and the old material is stale. Supply a signed witness set and policy, but omit/substitute the separate current policy SHA-256; confirm both fail. Supply the exact separately obtained pin and confirm only the exact terminal yields `trusted-match`.
...
5. Download a current-session backup, a separately selected saved-workspace backup, the exact current diagnostic receipt, and unsigned witness request; verify action names, targets, and non-authority labels.
...
10. Regenerate a completely internally valid replacement workspace/history while retaining the old local anchor; verify continuity failure and no in-place re-anchor. Then replace every browser-local store and unsigned receipt coherently; verify local checks alone cannot distinguish it but ordinary output still requires the exact signed chain/current policy pin. Delete/reorder/duplicate/truncate/fork/splice witnesses, replay an old/attacker policy, and use unknown/revoked/invalid keys; each must fail distinctly.
11. Review a synthetic backup from the old origin and create a recovery-transition review. Confirm it preserves actual v1/v2 envelope metadata and exact raw/payload/audit/optional continuity bindings while stating source reviewed, not activated; no inherited continuity/authority; and new lineage required. Regenerate an internally consistent review without the original process-local materials and verify comparison fails. On a genuinely clean supported device, separately create/save/reopen a new destination lineage, reconcile attachments, and retain source/destination observations; do not relabel the review itself as restore proof.
...
Acceptance: all targets, reviews, dispositions, policy pins, and baseline choices are explicit; freshness, identity, digest, topology, signature, policy, and checkpoint mismatches stop; recovery review never activates and the real destination uses a new lineage; local continuity/unsigned receipts and signed correspondence are not evidence authenticity; plaintext/origin, signer/key/policy-custody, clean-device, and external-governance boundaries are understood.
