...
Imported or restored content is diagnostic-only for ordinary outward artifacts pending independently governed corroboration outside IN KEEPING. The application does not convert that external decision into a local trusted status, so locally unverified content remains blocked until a governed revision removes or supersedes it. Each new review retains a complete, stable warning manifest bound to the exact source digest and parser profile; that manifest records what the implemented rules emitted, not truth or parser correctness. The ordinary UI output path requires a clean named save, an exact externally signed witness terminal under a separately obtained current policy digest, no unresolved unverified evidence, and a single-use saved-state lease. It renders from the reopened generation, re-digests the immutable artifact, and performs the final saved-state check and synchronous browser activation request inside one readonly IndexedDB transaction. Lower-level serializers are data transformers, not authority boundaries. Technical Reports, plaintext backups, unsigned receipts, and recovery-transition reviews remain explicit diagnostic, recovery, or comparison artifacts rather than institutionally authorized derivatives.
...
Workspace backups exclude the separately stored local continuity checkpoint. Reviewing a backup therefore cannot silently inherit the source browser's continuity status or activate a destination. A recovery-transition review preserves the actual envelope version, exact raw/payload/audit bindings, parser profile, and supplied continuity material while stating `source-reviewed-not-activated`, `continuityInherited: false`, `authorityInherited: false`, and a required new lineage. An unsigned receipt records content correspondence only; a signed terminal records correspondence only under the exact supplied policy pin. Neither establishes that the pin was independently obtained, raw-backup authenticity, source currency, custody, completeness, trusted time, attachments, clean-device conditions, or destination persistence. A real restore remains a separately observed operational transition.
...
6. For the exact clean named saved generation, verify a complete signed witness set against the exact current policy digest obtained through the approved separate channel. Generate an ordinary handoff only after the interface's signed-continuity, active-evidence, finding, immutable-artifact, and final readonly-transaction activation gates pass. Preserve the lossless native package alongside the approved handoff. The browser activation request does not prove browser/OS save completion or later currency.
