...
| Recorded interoperability evidence | Strict internal-consistency record for one source-controlled fixture profile and exact named receiver/package/diff/run claims. A derived `RECORDED_PASS` is not proof that the named execution occurred and is not product certification. | `in-keeping/interoperability-*` DTOs |
...
## Interoperability evidence chain

[`app/interoperability-evidence.ts`](../app/interoperability-evidence.ts) validates four strict, acyclic record types against a source-controlled fixture manifest. The current manifest registers only the synthetic versioned catalog-CSV profile and its two exact case/path definitions; another exchange profile cannot mint a recorded pass merely by declaring its own cases.

1. a receiver profile names the claimed product/version/environment and must carry the exact ordered cases derived from the matching source-controlled exchange definition;
2. a compatibility package contains one nonempty payload plus exact JSON loss and warning reports; all three text byte sequences are hashed, both reports bind the payload digest, and package metadata binds the manifest, full commit, build digest, exporter profile, and media type;
3. a semantic diff binds the exact profile and package, requires unique registered case/path observations, and rejects internally contradictory unchanged, loss-like, execution, network, and hazard classifications; and
4. a run binds that profile/package/diff plus the same manifest, build, and full commit claims, while each passing case requires a nonempty retained-evidence reference.

Assessment never trusts a caller-supplied subset of cases or a free-standing “pass.” It distinguishes `INVALID_RECORD`, `VERSION_OBSOLETE`, `FAILED`, `BLOCKED`, `INCOMPLETE`, and `RECORDED_PASS`. A pass requires exact registered observation coverage, a result for every derived case, a re-export descriptor where the manifest requires one, matching loss/warning details and classifications, no declared or unexpected loss or hazard, and current manifest/profile/build/commit pins. Unexpected loss or hazard derives `FAILED`; declared loss remains `INCOMPLETE` because the current fixture manifest has no allowed-loss policy. Raw-text validators apply byte limits and reject duplicate decoded JSON members and lone Unicode surrogates before semantic parsing. Malformed current pins return `INVALID_RECORD` rather than escaping the assessment contract as an exception.

These checks establish internal consistency of caller-supplied, digest-bound claims only. The hashes are not signatures; an evidence reference is not the referenced artifact; a re-export digest does not prove custody of its bytes; and none of these records proves execution, truth, operator identity, trusted time, product compatibility, or institutional acceptance. Retain and govern the actual receiving-product output, observation material, and acceptance record separately.
...
| CSV / TSV | `.csv`, `.tsv` / corresponding text type | Full canonical projection in version-1 columns with JSON list cells and explicit booleans | Versioned own-export round trip; JSON list/object cells receive the raw duplicate-member and Unicode-scalar scan before mapping. Formula-prefix detection sees through Unicode whitespace/format/bidi controls, compatibility sigils, and apostrophe chains while one reversible sentinel preserves legitimate apostrophe text. Unversioned list splitting is intentionally lossy. Exact downstream spreadsheet behavior remains external. |
...
Repository tests exercise all 20 normalized catalog record types through every own exporter/importer and exercise maximum cardinalities. Stable DOI identity is derived across typed identifiers, local/URI DOI carriers, and `https://doi.org`/`https://dx.doi.org` links in CSL, JSON-LD, RIS, BibTeX, MARC mnemonic/XML, MODS, and Dublin Core. Cross-record and destination collisions fail symmetrically; contradictory DOI carriers inside one record remain durable warnings rather than silent selection. MARC mnemonic/XML share conservative 020 terminal-qualifier and 024 indicator/`$2` rules. This establishes regression behavior inside the application, not arbitrary-source losslessness, external conformance, or real-world entity resolution. These checks cannot establish that an identifier is truthful, that two compatible records are the same entity, or that a source has authority.
...
On import, every description requires an explicit source reference code; generated XML IDs, component IDs, and ArchivesSpace `ref_id` values cannot substitute for it. Reference/identifier values reject line breaks and Unicode format/bidirectional controls. The supported EAD profile permits at most one repository identity carrier in each description. EAD4 requires nonempty text, exactly one digital-object URI carrier when present, and descriptive agents with exactly one label and the supported `creator` role. EAD3 and EAD 2002 require exactly one direct `name` or `corpname`; EAD3 retains one or more nonempty `part` elements as repeatable name evidence, while EAD 2002 requires nonempty name text. Duplicate, empty, or structurally competing carriers reject instead of selecting, role-rewriting, or concatenating silently.
