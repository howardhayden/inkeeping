...
An unsigned local continuity receipt is diagnostic comparison material only; matching one cannot unlock ordinary outward output. For an ordinary outward action, the exact named saved generation must instead match a P-256 signed witness chain whose terminal is named by a trust policy, and the policy must match the exact current SHA-256 obtained through a separate trust channel. The policy file cannot authorize itself. Save, rename, and reload clear this process-local external proof, and a later generation needs a newly signed terminal and current policy verification.

The interface renders from the reopened saved snapshot and constructs the immutable file before final activation. It then repeats the saved-state, external-continuity, evidence, and exact artifact-byte checks and opens one readonly IndexedDB transaction across the manifest, generation, and local-anchor stores. The storage layer accepts only that `File` plus an open/download disposition; it exposes no arbitrary activation callback. The synchronous browser activation request runs only after the exact saved workspace and anchor have matched while that snapshot remains held. Earlier writes are observed and later writes to those stores wait until the activation request returns. This fence does not prove that the browser or operating system persisted or opened the file, prevent a later write, provide continuing freshness, or grant publication authority. A Technical Report, plaintext workspace backup, and unsigned local receipt remain explicit diagnostic, recovery, or comparison artifacts.
...
Imports remain untrusted after structural review and explicit application. Controls include fatal UTF-8, byte and cardinality limits, a linear XML pre-scan, exact official XML namespaces, DTD/entity/processing-instruction rejection, exact DTO reconstruction, prototype-key rejection, public-HTTPS validation without requests, formula neutralization, exact successful-review binding, content-bound unverified disposition, apply-time destination revalidation, atomic IndexedDB writes, and state/payload digests. Structural validity and internal SHA-256 links detect defined mismatch; they do not establish truth, authorship, custody, completeness, authority, trusted time, or nonrepudiation. An externally signed continuity witness establishes only checkpoint correspondence under the selected, exactly pinned policy. IN KEEPING does not establish the policy's custody, the signer's institutional authority, the baseline's truth or completeness, or trusted chronology. Workspace backups exclude the separately stored local continuity checkpoint, and an unsigned continuity receipt is comparison evidence rather than a workspace backup or authenticity proof.
